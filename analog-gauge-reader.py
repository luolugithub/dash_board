# -*- coding: utf-8 -*-
# @Time    : 2019/12/24 下午4:05
# @Author  : luolu
# @Email   : argluolu@gmail.com
# @File    : analog-gauge-reader.py
# @Software: PyCharm
...
def calibrate_gauge(img_path):
    ...
    # detect circles restricting the search from 35-48% of the possible radii gives fairly good results across
    # different samples.  Remember that these are pixel values which correspond to the possible radii search range.
    # circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, 20, np.array([]), 100, 50, int(height * 0.35),
    #                            int(height * 0.48))
    circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, 20,
                               param1=100, param2=50, minRadius=300, maxRadius=600)
    # average found circles, found it to be more accurate than trying to tune HoughCircles parameters to get just the
    # right one
    ...
    cv2.imwrite('gauge-circles.jpg', img)
    ...
    cv2.imwrite('/home/luolu/PycharmProjects/BONC Cloudiip/result/gauge-calibration.jpg', img)
...
def get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r, image_path):
    ...
    cv2.imwrite('gauge-binary.jpg', dst2)
    ...
    cv2.imwrite('gauge-lines.jpg', img)
...
def main():
    gauge_number = 20
    file_type = 'jpg'
    img_path = "/home/luolu/PycharmProjects/BONC Cloudiip/images/gauge-2.jpg"
    ...
    min_angle, max_angle, min_value, max_value, units, x, y, r = calibrate_gauge(img_path)

    # feed an image (or frame) to get the current value, based on the calibration, by default uses same image as
    # calibration
    img = cv2.imread(img_path)
    val = get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r, img_path)
